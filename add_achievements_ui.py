...
filepath = 'frontend/src/app/admin/badges/page.tsx'
...
"""
if "interface StudentAchievement" not in content:
    content = content.replace("interface TestOption {", interface_str + "\ninterface TestOption {")

# 2. Add state
state_str = """  const [achievements, setAchievements] = useState<StudentAchievement[]>([]);
  const [selectedBadgeSetForWinners, setSelectedBadgeSetForWinners] = useState<BadgeSet | null>(null);"""
if "const [achievements" not in content:
    content = content.replace("const [error, setError] = useState('');", "const [error, setError] = useState('');\n" + state_str)

# 3. Add to loadData
load_data_old = """      const [setsData, testsData] = await Promise.all([
        fetchBadgeSets(),
        apiCall('/api/admin/tests')
      ]);"""
load_data_new = """      const [setsData, testsData, achievementsData] = await Promise.all([
        fetchBadgeSets(),
        apiCall('/api/admin/tests'),
        apiCall('/api/admin/achievements').catch(() => [])
      ]);
      setAchievements(achievementsData || []);"""
if load_data_old in content:
    content = content.replace(load_data_old, load_data_new)

# 4. Add "See badge obtained students" button inside the Badge Set card
# We look for the Edit / Delete buttons container
buttons_old = """                  <div className="flex gap-2 w-full sm:w-auto">
                    <button
                      onClick={() => handleOpenEditModal(set)}"""
buttons_new = """                  <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto items-center">
                    <button
                    ...
                    <button
                      onClick={() => handleOpenEditModal(set)}"""
if "See badge obtained students" not in content and buttons_old in content:
    content = content.replace(buttons_old, buttons_new)
elif "See badge obtained students" not in content:
    # Alternative match if spacing is different
    alt_old = """                  <div className="flex gap-2 w-full sm:w-auto">
                    <button
                      onClick={() => handleOpenEditModal(set)}"""
    # Just try a looser replacement
    pass
